...
import { PdfDoc, bytesToBase64 } from './pdf.js';
import { GRAPH } from './graph.js';

// ---------------------------------------------------------------------------
// 1) RENDER: record -> PDF bytes
// ---------------------------------------------------------------------------
export function renderDailyReportPdf(rec) {
  const doc = new PdfDoc();
  const projLabel = [rec.projectName, rec.projectId].filter(Boolean).join('  #');
  doc.title('Pier Foundations - Daily Field Report', projLabel || 'Daily Field Report');
  ...
  if (owned.length || rental.length) {
    ...
    if (owned.length) {
      doc.text('Owned', { bold: true, size: 10 });
      for (const e of owned) {
        const hrs = (e.hours != null && e.hours !== '') ? `${e.hours} hr meter` : '';
        doc.row('  ' + (e.machine || '-'), hrs, { rightW: 110 });
      }
    }
    if (rental.length) {
      doc.text('Rental', { bold: true, size: 10 });
      for (const e of rental) {
        const hrs = (e.hours != null && e.hours !== '') ? `${e.hours} hr meter` : '';
        doc.row('  ' + (e.category || '-'), hrs, { rightW: 110 });
      }
    }
  }
  ...
  doc.text(`Generated by the PF Platform on submit - ${gen}. Field operations record - no financial data.`, { size: 8 });
