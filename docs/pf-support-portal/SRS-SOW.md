...
- Verification (2026-07-21): `pf-platform` deploy compiled Worker successfully + Functions bundle uploaded; prod root returns `401` Basic-Auth realm "PF Operations Platform" (gate intact, expected); `/login.html` → 308 (login page still public, shell not broken); the Help anchor with `target="_blank"` + `href="https://pf-support.pages.dev/"` confirmed in the deployed `platform/index.html`.
- **RELOCATED to top of sidebar (2026-07-21, Melanie):** moved the single Help / Support anchor from the bottom of the "Estimating & Tools" cluster to the very TOP of `<nav class="sidebar-nav">`, above the first section group (PF Dashboard). Given clean prominent styling (azure `#006DB0` text, light `#eaf4fb` pill with `#cfe3f2` border) rather than the red dev-doc color, so it reads as a friendly help item for non-technical users. Removed the old buried copy — exactly ONE Help occurrence remains. Deployed via `./deploy.sh` (canonical=production verified); authed prod HTML shows the Help anchor before the first nav-section group; data/ + sync/ working tree unchanged (18 test-data changes preserved).
